...
def mutate_all(prot, res ,num ):
    if res != "ALA":
        ...
        cmd.frame(1)
        cmd.get_wizard().apply()
        cmd.save(f"../files/mutant_{res}_{prot[1]}_ALA.pdb","obj_%s"%(num))
     #   cmd.save("../files/mutant_all_residues.pdb","obj_%s"%(num))
        ...
        cmd.frame(1)
        cmd.get_wizard().apply()
        cmd.save(f"../files/mutant_{res}_{prot[1]}_GLY.pdb","obj_%s"%(num))
    #    cmd.save("../files/mutant_all_residues.pdb","obj_%s"%(num)) 
...
for prot in prot_list:
    ...
    for num,res in enumerate ( residues_list ) :
        num +=1
        if num == choice:   
            mutate_all(prot, res, num=num)
